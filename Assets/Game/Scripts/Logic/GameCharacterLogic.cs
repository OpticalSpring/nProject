...
public class GameCharacterLogic : MonoBehaviour
{
    public static bool CheckObstacle(GameObject gameCharacter)
    {
        ...
        if (Physics.SphereCast(gameCharacter.transform.position + new Vector3(0, 1, 0), 0.2f, gameCharacter.transform.forward, out rayHit, 0.5f, mask))
        {
            return true;
        }
        else
    ...
    public static Vector3 GetSlidingVector(GameObject gameCharacter, float speed)
    {
        RaycastHit rayHit;
        int mask = 1 << 2;
        mask = ~mask;
        ...
        Vector3 S;
        Vector3 V = Vector3.Normalize(gameCharacter.transform.forward);
        Vector3 nPoint = gameCharacter.transform.position;
        if (Physics.SphereCast(gameCharacter.transform.position, 0.2f, gameCharacter.transform.forward, out rayHit, 0.8f, mask))
        {
